...
const Contact = () => {
  ...
  const handleInputChange = (field, value) => {
    ...
    // }
    if (field === 'phone') {
  processedValue = value.replace(/\D/g, "");   // remove non-numbers
  processedValue = processedValue.slice(0, 10); // limit 10 digits
}
  ...
  try {
    const response = await fetch("http://localhost:8000/api/save-contact/", {
      ...
      body: JSON.stringify({
        ...
        phone: formData.phone.replace("+91", "").replace(/\s/g, ""),
